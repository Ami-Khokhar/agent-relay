...
| MCP over stdio | Coding agents that can discover and call tools | Chosen as a thin interface over the HTTP API. It uses `list_agents`, `delegate`, `wait_task`, `get_task`, `list_tasks`, `list_sessions`, and `cancel_task`. |
