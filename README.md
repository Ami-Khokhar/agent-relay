...
Updating: `bash scripts/update.sh` fast-forwards the checkout at `~/.local/share/agent-relay` to `main` and restarts the service only when the code changed, then verifies `/healthz`. Tasks and sessions live in memory, so the restart is skipped while tasks are in flight unless `--force` is passed (`--no-restart` updates the files only). `bash scripts/install-service.sh --with-update-timer` additionally installs a scheduled update that runs `update.sh` every 6 hours (`AGENT_RELAY_UPDATE_INTERVAL_SECS` overrides the period); `install-service.sh --uninstall` removes both. MCP clients and the skill need no reconfiguration: the tool list refreshes on each client's next session start.
